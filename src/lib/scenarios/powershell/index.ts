  ...
  tasks: [
    {
      ...
      details:"In **Bash**, quando fai ps aux, il **comando** ti sputa fuori del testo che poi devi tagliare a mano con awk o cut per estrarre quello che ti serve. In **PowerShell**, invece, Get-Process ti restituisce oggetti processo con proprietà come Name, Id, CPU: basta scrivere .CPU e hai il valore senza parsing.\n\nQuesto approccio a oggetti rende gli script più corti, più leggibili e meno soggetti a errori quando cambia il formato dell'output. È il motivo per cui gli amministratori **Windows** preferiscono **PowerShell** per l'automazione.\n\nIn questo primo esercizio, ti mostreremo alcune affermazioni sulle due **shell**: dovrai classificarle come tipiche di **Bash**, di **PowerShell** o comuni a entrambe.\n\nLa differenza più importante emerge nella pipeline: **Bash** trasferisce normalmente righe di testo, **PowerShell** conserva proprietà e tipi degli oggetti. Classificando gli esempi, cerca quindi di capire se il **comando** tratta una semplice stringa oppure un elemento strutturato che può essere interrogato.",
    ...
    },
    {
      ...
      details:"I verbi ammessi sono un elenco fisso e ben definito: Get (leggi), Set (modifica), New (crea), Remove (elimina), Start, Stop, e pochi altri. I sostantivi descrivono l'oggetto: Process, Service, Item (file/cartella), ChildItem (contenuto di una cartella).\n\nQuesta uniformità è una delle grandi forze di **PowerShell**: chi impara pochi verbi e pochi sostantivi riesce già a lavorare con moltissimi comandi. Non c'è più bisogno di ricordare nomi arbitrari come ls, rm, kill, mkdir.\n\nNella simulazione, per ogni obiettivo dovrai comporre il cmdlet corretto scegliendo il Verbo e il Sostantivo giusti.\n\nIl nome di un cmdlet descrive chiaramente l’azione e l’oggetto interessato. Separando verbo e sostantivo puoi costruire il **comando** per logica: prima decidi se vuoi leggere, creare o fermare qualcosa, poi scegli la risorsa su cui agire.",
    ...
    },
    {
      ...
      details:"Quando non ricordi il nome esatto di un cmdlet, Get-Command -Verb Get ti mostra tutto quello che inizia per Get. Quando invece vuoi capire cosa fa un cmdlet e quali parametri accetta, Get-Help Get-Process ti dà una descrizione sintetica; con -Examples ottieni anche gli esempi pratici.\n\nQuesta capacità di auto-documentarsi è uno dei motivi per cui **PowerShell** è considerata beginner-friendly: non devi memorizzare tutto, ti basta sapere come chiedere.\n\nNella simulazione hai un mini-**terminale** **PowerShell**: prova a esplorare l'elenco dei comandi e a leggere l'aiuto di almeno un cmdlet.\n\nGet-Command risponde alla domanda «quali comandi esistono?», mentre Get-Help spiega come usarne uno specifico. Provali in sequenza: prima individua un cmdlet utile, poi consulta sintassi ed esempi senza dover uscire dalla **shell**.",
    ...
    },
    {
      ...
      details:"Immagina di voler vedere i tre processi che consumano più CPU: prendi tutti i processi con Get-Process, tieni solo quelli con CPU maggiore di 10 usando Where-Object, li ordini per CPU decrescente con Sort-Object e infine prendi i primi 3 con Select-Object -First 3.\n\nOgni stadio riceve oggetti dal precedente e passa oggetti al successivo. Non c'è mai bisogno di parsare testo: la **variabile** speciale $_ rappresenta l'oggetto corrente dentro Where-Object e ForEach-Object.\n\nNella simulazione devi rimettere in ordine i quattro stadi della pipeline per raggiungere l'obiettivo.\n\nOgni stadio della pipeline restringe o organizza il risultato precedente. Partire dai dati completi, filtrare gli elementi utili, ordinarli e infine limitarli crea una sequenza leggibile e riduce il lavoro necessario nei passaggi successivi.",
    ...
    },
    {
      ...
      details:"Assegni un valore con l'uguale, richiami la **variabile** scrivendo $nome. La cosa interessante è che puoi chiamare il metodo .GetType() su qualsiasi variabile per scoprire di che tipo è: Int32, String, DateTime, ecc.\n\nQuesto è diverso da **Bash**, dove tutte le **variabili** sono di fatto stringhe. In **PowerShell** l'informazione sul tipo è sempre disponibile, e questo abilita autocompletamento, controllo errori e conversioni automatiche.\n\nNel mini-**terminale** prova ad assegnare qualche **variabile** e a ispezionare il tipo con .GetType().\n\n**PowerShell** conserva insieme il valore e il suo tipo, quindi una **variabile** numerica offre possibilità diverse da una stringa. Ispezionare GetType e alcune proprietà rende visibile questa differenza e aiuta a prevedere quali operazioni saranno accettate.",
    ...
    },
    {
      ...
      details:"Get-ChildItem C:\\ ti mostra i file, Get-ChildItem Env: ti mostra le **variabili** d'ambiente, Get-ChildItem Variable: ti mostra le variabili **PowerShell** attive, Get-ChildItem `HKLM`:\\Software elenca chiavi di registro.\n\nQuesta astrazione unica (chiamata provider) è potentissima: impari un solo **comando** e lo usi ovunque. È anche perfetta per scripting di ricognizione, dove devi ispezionare rapidamente parti diverse del sistema.\n\nNella simulazione ti chiediamo di elencare due provider diversi.\n\nI provider presentano risorse diverse attraverso una navigazione comune. Cambiando percorso puoi esplorare file, ambiente, **variabili** o registro con gli stessi cmdlet, ma i contenuti ottenuti rappresentano parti molto diverse del sistema.",
    ...
    },
    {
      ...
      details:"L'espressione 1..5 crea una sequenza da 1 a 5. Se la mandi in pipeline a ForEach-Object { \"server-$_\" }, dentro il blocco la **variabile** speciale $_ rappresenta l'elemento corrente. Il risultato è una lista di stringhe: server-1, server-2, ...\n\nQuesto pattern è usato ovunque: generare nomi, iterare su file, chiamare un cmdlet su ogni riga di un CSV. Padroneggiarlo significa poter scrivere script di automazione in due righe.\n\nNella simulazione sposta lo slider per scegliere quanti elementi generare, poi esegui e osserva l'output arrivare uno alla volta.\n\nForEach-Object riceve gli elementi uno alla volta dalla pipeline e applica a ciascuno lo stesso blocco. La **variabile** $_ cambia valore a ogni passaggio: seguirla nell’output rende visibile il funzionamento dell’intera iterazione.",
    ...
    },
    {
      ...
      details:"I livelli principali sono quattro: Restricted (nessuno script), AllSigned (solo script firmati digitalmente), RemoteSigned (locali liberi, remoti solo firmati), Bypass (tutto permesso, nessun controllo).\n\nLa scelta giusta dipende dal contesto: una workstation aziendale userà RemoteSigned, un server bloccato userà AllSigned, una VM di laboratorio può stare in Bypass. Restricted è il default su molti sistemi client.\n\nNella simulazione ti presentiamo quattro scenari: assegna a ciascuno la policy più appropriata.\n\nLe policy riducono soprattutto l’esecuzione involontaria di script non affidabili; non sostituiscono antivirus, permessi o controllo degli accessi. Associa ogni ambiente al livello adeguato considerando provenienza degli script, firme richieste e libertà operativa necessaria.",
    ...
    },
    {
      ...
      details:"Non serve un punteggio perfetto: se sbagli qualcosa torna al task corrispondente e rileggi la spiegazione. È un modo utile per capire dove sei sicuro e dove no.\n\nIl quiz copre tutto quello che abbiamo visto: dalla filosofia degli oggetti fino allo script parametrico. Rispondi con calma, senza fretta.\n\nBuon lavoro!\n\nNel quiz ritroverai la stessa logica osservata nelle attività: nomi coerenti, oggetti che scorrono nella pipeline e percorsi gestiti dai provider. Leggi ogni **comando** per parti e chiediti quale tipo di risultato passerà allo stadio seguente.",
