...
export const bashScenario: Scenario = {
  ...
  tasks: [
    ...
    },
    {
      ...
      details:"Scrivere un **comando** è un po' come comporre una frase in italiano. C'è un verbo (il comando), un avverbio che spiega come farlo (l'**opzione**) e un complemento oggetto su cui agire (l'**argomento**). Se impari questa grammatica, non dovrai mai più imparare a memoria migliaia di comandi diversi.\n\nAd esempio, se vuoi vedere i file in una cartella in modo dettagliato, scriverai ls -l /Documenti. Qui ls è l'ordine di elencare, -l è l'**opzione** che chiede il formato lungo, e /Documenti è il posto dove guardare. Molte opzioni possono essere unite insieme, come usare -la per vedere anche i file nascosti.\n\nIn questa esercitazione, dovrai mettere in ordine i pezzi di un **comando**. Questo ti aiuterà a familiarizzare con la struttura logica che **Bash** si aspetta di ricevere ogni volta che premi Invio sulla tastiera.\n\nIl primo elemento indica quale programma avviare, le opzioni ne modificano il comportamento e gli argomenti specificano su cosa lavorare. Imparare questa struttura ti permette di leggere comandi mai visti prima senza considerarli una lunga stringa incomprensibile.",
    ...
    },
    {
      ...
      details:"A volte vuoi che uno script faccia la stessa operazione ma su file o utenti diversi ogni volta. Invece di modificare il file ogni volta, puoi usare gli argomenti. Quando lanci uno script seguito da una parola, **Bash** assegna quella parola a una **variabile** speciale chiamata $1.\n\nSe scrivi due parole dopo il nome dello script, la seconda finirà in $2, e così via. Se vuoi riferirti a tutti gli argomenti insieme, puoi usare il simbolo $@. È un modo fantastico per creare strumenti che si adattano a quello che scrivi sulla riga di **comando** al momento del lancio.\n\nOggi proverai a lanciare uno script passando diversi nomi come input. Osserverai come lo script cambia il suo comportamento in base a ciò che scrivi dopo il **comando**, rendendolo molto più versatile.\n\nGli argomenti permettono allo stesso script di lavorare ogni volta su dati differenti. Osserva come la posizione sulla riga di **comando** determina il nome speciale $1 o $2, mentre $@ raccoglie l’intero insieme nell’ordine ricevuto.",
    ...
    },
    {
      ...
      details:"Immagina di dover scansionare cento indirizzi IP o rinominare mille fotografie. Farlo a mano richiederebbe ore, ma con un ciclo for bastano tre righe di codice. Il ciclo prende una lista di elementi e, uno per uno, li assegna a una **variabile** temporanea per poi eseguire un **comando**.\n\nLa struttura tipica è: per ogni elemento in questa lista, fai questa azione, fine. **Bash** continuerà a girare finché non avrà finito tutti gli elementi dell'elenco. È una potenza incredibile racchiusa in una sintassi molto semplice da leggere una volta capito il meccanismo.\n\nIn questa simulazione, potrai decidere quanti elementi far processare al tuo ciclo. Vedrai come lo script ripete ordinatamente le istruzioni per ogni voce, mostrandoti l'output in tempo reale per ogni singolo giro.\n\nIl ciclo for prende un elemento alla volta da una lista e ripete lo stesso blocco usando quel valore. Segui l’output riga per riga per vedere come cambia la **variabile** temporanea mentre la struttura del **comando** resta identica.",
    ...
    },
    {
      ...
      details:"La filosofia di **Linux** è creare piccoli strumenti che fanno bene una sola cosa. La magia avviene quando li colleghi con la pipe, il simbolo della barra verticale |. Con la pipe, l'uscita di un **comando** diventa l'ingresso del successivo, permettendoti di filtrare e trasformare i dati in modi complessi.\n\nOltre a collegare i comandi, puoi anche decidere dove mandare il risultato finale. Usando il simbolo di maggiore >, puoi dire a **Bash** di non stampare il testo sullo schermo, ma di scriverlo dentro un file. Se invece usi due simboli >>, aggiungerai il testo alla fine del file senza cancellare quello che c'era prima.\n\nOggi costruirai una vera e propria catena di montaggio dati. Partirai da un elenco grezzo, userai un filtro per cercare solo quello che ti serve e infine salverai il risultato pulito in un file di testo.\n\nLa pipe collega due programmi senza creare file intermedi: il risultato del primo diventa subito il materiale di lavoro del secondo. La redirezione decide invece dove conservare l’uscita finale, sostituendo oppure aggiungendo contenuto a un file.",
    ...
    },
    {
      ...
      details:"Abbiamo percorso un lungo cammino: dalla scoperta della **shell** alla creazione di script con cicli e condizioni. Questo quiz non serve a darti un voto, ma a farti capire se c'è qualche concetto che ha bisogno di una ripassata veloce prima di proseguire il tuo viaggio.\n\nLe domande coprono tutto quello che abbiamo visto: la struttura dei comandi, l'uso del dollaro per le **variabili**, il funzionamento delle pipe e la logica degli script. Non avere fretta e rifletti su ogni risposta basandoti sulle simulazioni che hai appena completato.\n\nAffronta il quiz con serenità. Se una risposta non ti convince, prova a ricordare gli esempi pratici che abbiamo fatto insieme. Una volta finito, avrai una solida base su cui costruire le tue future competenze di **ethical hacker**.\n\nLe risposte corrette dipendono dalla funzione dei simboli, non soltanto dal loro aspetto. Ricostruisci cosa entra in un **comando**, cosa viene elaborato e dove finisce l’output: questo ragionamento aiuta a distinguere opzioni molto simili.",
