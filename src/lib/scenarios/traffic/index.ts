...
export const trafficScenario: Scenario = {
  ...
  tasks: [
    ...
    },
    {
      ...
      details:"Il riquadro Packet List mostra una riga per pacchetto con numero, tempo, IP sorgente, IP destinazione, protocollo e una breve descrizione. Il riquadro Packet Details espande il pacchetto selezionato mostrando tutti i livelli in modo gerarchico. Il riquadro Packet Bytes mostra gli stessi dati in esadecimale e ASCII.\n\nCliccando su un campo dei dettagli, **Wireshark** evidenzia i byte corrispondenti nel terzo riquadro. È il modo migliore per capire dove finisce un livello e dove inizia il successivo.\n\nNella simulazione clicca sulle tre aree per identificarle.\n\nLe tre aree raccontano lo stesso pacchetto con livelli di dettaglio differenti. Parti dalla riga riassuntiva, espandi il protocollo che ti interessa e passa ai byte grezzi soltanto quando devi verificare esattamente come un valore è stato registrato.",
    ...
    },
    {
      ...
      details:"Sintassi essenziale: ip.addr == `10.0.0.5` filtra per un IP, tcp.port == 443 filtra per porta, **http** mostra solo pacchetti HTTP, **dns** solo query DNS. Puoi combinarli con and, or, not. Esempio: ip.addr == `10.0.0.5` and tcp.port == 443.\n\nAttenzione: eq usa il doppio uguale (==), non uno solo. Se sbagli sintassi, la barra dei filtri diventa rossa.\n\nNella simulazione componi il filtro giusto per raggiungere l'obiettivo di ogni scenario.\n\nUn buon filtro nasce da una domanda precisa: quale host, porta o protocollo vuoi isolare? Costruisci prima la condizione più semplice, controlla che funzioni e aggiungi un secondo criterio con and soltanto quando serve restringere ulteriormente i risultati.",
    ...
    },
    {
      ...
      details:"Il client manda un pacchetto con il flag SYN al server. Il server risponde con SYN + ACK. Il client conferma con un ACK finale. Da quel momento la connessione è aperta e i dati possono scorrere.\n\nRiconoscere questo pattern è fondamentale: quando fai troubleshooting, un **handshake** che non si completa vuol dire firewall che blocca, server spento o timeout. Anche molti port scan (SYN scan) sfruttano solo il primo pacchetto.\n\nNella simulazione osserva l'animazione e rimetti in ordine i tre pacchetti.\n\nI tre messaggi confermano che entrambe le parti sono raggiungibili e pronte a scambiare dati. Se compare solo SYN, il client non riceve risposta; se arriva SYN/ACK ma manca l’ACK finale, la connessione non viene completata dal client.",
    ...
    },
    {
      ...
      details:"È utilissima per **HTTP** non cifrato: vedi la richiesta del client (GET, POST, headers, body) e la risposta del server (status code, headers, HTML) come testo leggibile. Su traffico **HTTPS**, invece, vedi solo dati cifrati: serve la chiave di sessione per decifrarli.\n\nQuesta funzione è indispensabile in analisi forense: se un attaccante ha esfiltrato dati via **HTTP**, qui li vedi in chiaro.\n\nNella simulazione ordina i frammenti della conversazione per ricostruire lo scambio.\n\nI pacchetti di una connessione possono arrivare separati, ma Follow Stream li riordina secondo la sequenza TCP. Ricostruisci prima la richiesta completa del client e poi la risposta del server, distinguendo intestazioni e contenuto del messaggio.",
    ...
    },
    {
      ...
      details:"Nei form web, le credenziali di login vengono inviate al server nel body di una richiesta POST, in genere in formato username=xxx&password=yyy. Basta selezionare la richiesta in **Wireshark** e guardare il **payload** per trovarle.\n\nQuesto è uno dei motivi per cui oggi tutti i siti seri usano **HTTPS**: senza cifratura, sniffare le credenziali è banale, anche per un attaccante alle prime armi.\n\nNella simulazione analizza la richiesta POST e individua le credenziali corrette.\n\nNel corpo della richiesta, ogni coppia chiave-valore rappresenta un campo compilato dall’utente. Individuare password e nome utente in chiaro dimostra concretamente che **HTTP** protegge il trasporto del dato in nessun modo: la difesa corretta è **HTTPS**.",
    ...
    },
    {
      ...
      details:"I comandi base: **tshark** -r `file.pcap` legge un file, -Y \"filtro\" applica un filtro **Wireshark**, -T fields -e ip.src -e ip.dst estrae solo alcuni campi, -c 10 limita il numero di pacchetti mostrati.\n\nUn esempio potente: **tshark** -r `traffic.pcap` -Y \"**http**.request\" -T fields -e ip.src -e http.host ti dà una lista di chi ha contattato quali domini via HTTP, pronta da passare a sort o uniq.\n\nNella simulazione hai un mini-**terminale**: prova alcuni comandi **tshark** su un **pcap** simulato.\n\n**Tshark** applica gli stessi filtri di **Wireshark** senza interfaccia grafica. L’**opzione** -r sceglie la cattura, -Y restringe i pacchetti e -T fields con -e estrae soltanto le colonne necessarie, producendo un risultato facile da automatizzare.",
    ...
    },
    {
      ...
      details:"Non serve il punteggio perfetto: se sbagli, torna al task corrispondente e rileggi la spiegazione. Il quiz copre tutto quello che abbiamo visto: dagli strati di un pacchetto agli strumenti da riga di **comando**.\n\nBuon lavoro!\n\nPer rispondere, segui un metodo di analisi: identifica il livello o lo strumento coinvolto, poi collega la domanda all’azione corretta. Questo evita di confondere concetti vicini come filtro, ricostruzione dello stream ed estrazione dei campi.",
