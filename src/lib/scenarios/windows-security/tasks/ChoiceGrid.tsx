...
export function ChoiceGrid({ items, markComplete, isComplete, instruction, success, minimum }: TaskContext & { items: ChoiceItem[]; instruction: string; success: string; minimum?: number }) {
  ...
  return <div>
    <div className="space-y-3">{items.map((item, index) => <div key={item.text} className="rounded-lg border border-border bg-surface p-4">
      ...
      <div className="flex flex-wrap gap-2">{item.options.map((option, optionIndex) => {
    ...
    <div className="mt-4 flex items-center gap-3">{!checked ? <Button type="button" disabled={answers.some((answer) => answer === null)} onClick={verify}>Verifica risposte</Button> : <Button type="button" variant="outline" onClick={reset}><RotateCcw />Riprova</Button>}{checked && <span className="font-mono text-xs text-muted-foreground">{score}/{items.length}</span>}</div>
